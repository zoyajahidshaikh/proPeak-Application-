...
exports.leaveApplicationSave = async (request, response) => {
    try {
      ...
      const { leaveApplication } = request.body;
  
      if (!leaveApplication) {
        throw new Error("Invalid request body: leaveApplication data missing.");
      }
      ...
      if (config.prodMode === "ON") {
        ...
        const reportingManagerId = loggedInUser?.reportingManagerId;
  
        if (!reportingManagerId) {
          throw new Error("Reporting manager not found!");
        }
  
        const reportingManager = await userModel.findOne(
          { _id: reportingManagerId },
          {}
        );
  
        toEmail = reportingManager?.email;
      } else {
      ...
      if (mailResponse.response) {
        console.error("Error occurred while sending email to " + mailOptions.to);
      ...
      console.error("Error in leaveApplicationSave:", error);
